                ...
                <form id="form" name="form" enctype="multipart/form-data">
                    <div class="form-group">
                        ...
                        @foreach ($language_data as $l_data)
                            ...
                            <textarea name="description[{{$l_data->code}}]" class="form-control description_{{$l_data->code}}" id="ckeditor_{{$l_data->code}}" placeholder="Description" required></textarea><br>

                            <span>How does This Program work?_{{$l_data->code}} </span>
                            <textarea name="program_work[{{$l_data->code}}]" class="form-control" id="ckeditor_program_{{$l_data->code}}" placeholder="How does This Program work?"></textarea><br>
                            ...
                            <textarea name="break_down[{{$l_data->code}}]" class="form-control" id="ckeditor_break_{{$l_data->code}}" placeholder="Here is a step-by-step breakdown for the process with us"></textarea><br>

                            <span>Main Advantages_{{$l_data->code}} <span class="label-title">*</sapn></span>
                            <textarea name="main_advantage[{{$l_data->code}}]" class="form-control" id="ckeditor_advantage_{{$l_data->code}}" placeholder="Main Advantages"></textarea><br>

                            <span>Candidate Requirements_{{$l_data->code}} </span>
                            <textarea name="requirements[{{$l_data->code}}]" class="form-control" id="ckeditor_requirements_{{$l_data->code}}" placeholder="Candidate Requirements"></textarea><br>
                            ...
                            <textarea name="time_frame[{{$l_data->code}}]" class="form-control" id="ckeditor_frame_{{$l_data->code}}" placeholder="Time Frame"></textarea><br>

                            <span>Service Cost_{{$l_data->code}} </span><br>
                            <input required type="file" name="cost_image[{{$l_data->code}}]" class="form-control cost_image_{{$l_data->code}}" id="cost_image"><br>
                        @endforeach
...
<script>
    ...
    $('document').ready(function () {
        ...
        @foreach ($language_data as $l_data)
            CKEDITOR.replace('ckeditor_{{$l_data->code}}', {
                filebrowserUploadMethod: 'form'
            });
            CKEDITOR.replace('ckeditor_program_{{$l_data->code}}', {
                filebrowserUploadMethod: 'form'
            });
            CKEDITOR.replace('ckeditor_break_{{$l_data->code}}', {
                filebrowserUploadMethod: 'form'
            });
            CKEDITOR.replace('ckeditor_advantage_{{$l_data->code}}', {
                filebrowserUploadMethod: 'form'
            });
            CKEDITOR.replace('ckeditor_requirements_{{$l_data->code}}', {
                filebrowserUploadMethod: 'form'
            });
            CKEDITOR.replace('ckeditor_frame_{{$l_data->code}}', {
                filebrowserUploadMethod: 'form'
            });
        @endforeach
        ...
        $('#saveBtn').click(function (e) {
            ...
            @foreach ($language_data as $l_data)
                form_data.append('description[{{$l_data->code}}]', CKEDITOR.instances['ckeditor_{{$l_data->code}}'].getData());
                form_data.append('program_work[{{$l_data->code}}]', CKEDITOR.instances['ckeditor_program_{{$l_data->code}}'].getData());
                form_data.append('break_down[{{$l_data->code}}]', CKEDITOR.instances['ckeditor_break_{{$l_data->code}}'].getData());
                form_data.append('time_frame[{{$l_data->code}}]', CKEDITOR.instances['ckeditor_frame_{{$l_data->code}}'].getData());
                form_data.append('main_advantage[{{$l_data->code}}]', CKEDITOR.instances['ckeditor_advantage_{{$l_data->code}}'].getData());
                form_data.append('requirements[{{$l_data->code}}]', CKEDITOR.instances['ckeditor_requirements_{{$l_data->code}}'].getData());
