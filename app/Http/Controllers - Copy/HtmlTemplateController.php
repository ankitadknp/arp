...
class HtmlTemplateController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = [
            ...
            'content'    => 'content.view_html_template',
            'title'      => 'Html Template'
        ];

        $checkAuth = \Auth::user()->level;
        if ($request->ajax()) {
            $q_brand = VisaType::with('brand')->select('*')->orderByDesc('created_at');
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            for($i = 1; $i <count($request->file('image')); $i++) {
        ...
        if(isset($single) && $single){
            setActivityLog('Visa Type Updated [ID: ' . $request->id . ', ' . $request->email  . ' -> '.$request->name . ']',json_encode($reqInt),activityEnums('visa-type'),$request->id,\Auth::user()->id);
        }else{
            setActivityLog('New Visa Type Added [ID: ' . $rec->id . ', ' . $request->email  . ' -> '.$request->name . ']',json_encode($reqInt),activityEnums('visa-type'),$rec->id,\Auth::user()->id);
        }
